...
import type { ReferentialAction } from '../../domain/table-designer'
    ...
    <section className="flex flex-col gap-3">
      <header className="flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700 transition-colors hover:bg-indigo-100"
        >
          <Plus size={11} /> Add Foreign Key
        </button>
      </header>

      {pendingModel.foreignKeys.length === 0 && (
        <p className="text-xs text-slate-400 italic">No foreign keys defined.</p>
      )}

      {pendingModel.foreignKeys.map((fk) => (
        <div
          key={fk.id}
          className="rounded-lg border border-indigo-200 bg-indigo-50/30 p-3 space-y-3"
        >
          ...
          <div>
            ...
            <div className="mt-1 flex flex-wrap gap-1.5">
              ...
              <button
                ...
                className="rounded-full border border-dashed border-slate-300 px-2 py-1 text-[10px] text-slate-500 hover:border-indigo-300 hover:text-indigo-600"
              >
                + Column
              </button>
            </div>
