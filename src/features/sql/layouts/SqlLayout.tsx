...
import { WorkspaceToolbar } from '../../data-explorer/components/shared/WorkspaceToolbar'
import type { ToolbarItem } from '../../data-explorer/components/shared/WorkspaceToolbar'
import { DeleteTableModal } from '../../data-explorer/components/DeleteTableModal'
import { ExportDataModal } from '../../data-explorer/components/ExportDataModal'
import { DataOperationModal } from '../../data-explorer/components/DataOperationModal'
import { TableDesignerModal } from '../../data-explorer/components/table-designer/TableDesignerModal'
import { isSqlConnectionType, getConnPayload } from '../../data-explorer/utils'
...
export function SqlLayout() {
  ...
  const treeData = connectionId ? explorerData.treeDataMap[connectionId] : null
  const databases = useMemo(() => treeData?.databases ?? [], [treeData?.databases])
  const allSchemas = useMemo(
    () => databases.flatMap((db) => db.schemas ?? []),
    [databases],
  )
  ...
  const subNavItems = [
    ...
    { label: 'ERD', path: `/sql/${connectionId}/erd`, active: activeSection === 'erd' },
  ]
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            {databases.length > 0 && (
              <select
                ...
                onChange={(e) => queryExecution.onQueryDatabaseChange(e.target.value)}
                className="rounded-md border border-outline-variant bg-surface-container-low px-2 py-1 text-xs text-on-surface focus:outline-none focus:ring-1 focus:ring-primary"
              >
                {databases.map((db) => (
            ...
            {connection.type === 'postgresql' && allSchemas.length > 0 && (
              <select
                value={currentSchema}
                onChange={(e) => queryExecution.onQuerySchemaChange(e.target.value)}
                className="rounded-md border border-outline-variant bg-surface-container-low px-2 py-1 text-xs text-on-surface focus:outline-none focus:ring-1 focus:ring-primary"
              >
      ...
      {/* ── Tab bar ── */}
      <div className="flex items-center gap-1 border-b border-outline-variant bg-surface-variant px-3 py-1">
      ...
      {deleteTableTarget && (
        <DeleteTableModal
          ...
          onDelete={async (tableName, cascade) => {
            ...
            const { executeSql } = await import('../../../services/clients/sql')
            const sql =
